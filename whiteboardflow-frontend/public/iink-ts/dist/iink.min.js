...
downloadAsSVG(i=!1){
    ...
    sendPNGToServer(i=!1){
        ...
        l.onload=()=>{
            ...
            i.getContext("2d").drawImage(l,0,0),URL.revokeObjectURL(a);
            const r=i.toDataURL("image/png").replace("image/png","image/octet-stream");
            // this.triggerDownload(this.getExportName("png"),r)
            ...
            fetch('/upload', {
                ...
                // },
                body: r  // Sending the SVG blob directly to the server
            })
                .then(response => {
                    if (response.ok) {
                        console.log('SVG successfully sent to the backend');
                    } else {
                        console.error('Failed to send SVG');
                    }
                    URL.revokeObjectURL(a); // Clean up the blob URL after sending
                })
                .catch(error => {
                    console.error('Error sending SVG to the backend:', error);
                    URL.revokeObjectURL(a); // Ensure clean up in case of error
                });
    ...
    sendSVGToServer(i = false) {
    const symbols = i ? this.model.symbolsSelected : this.model.symbols;
    const bounds = this.getSymbolsBounds(symbols);
    const svgBlob = this.buildBlobFromSymbols(symbols, bounds);
    const svgUrl = URL.createObjectURL(svgBlob);

    // return (this.getExportName("svg"),svgBlob);

    // axios.post("/upload", svgBlob).catch((error) => {
    ...
    fetch('/upload', {
        ...
        //     'Content-Type': 'application/octet-stream',
        // },
        body: svgBlob  // Sending the SVG blob directly to the server
    })
        .then(response => {
            if (response.ok) {
                console.log('SVG successfully sent to the backend');
